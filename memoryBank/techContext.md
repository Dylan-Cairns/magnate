...
- Active Python entrypoints:
  ...
  - `python -m scripts.run_td_loop_selfplay` (primary post-bootstrap loop: shorter collect/train cadence, mixed td-search-heavy collection, promoted opponent pool usage, and dual promotion gates vs fixed `search` baseline plus incumbent `td-search`; replay regime `chunk-local-selfplay-mixed`)
