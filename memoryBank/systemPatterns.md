...
- Pre-commit presentation reducers should construct visible mutations from the
  ...
  narrowly justified reveal step such as draw-card deck state.
...
- `TokenChip` owns its tooltip target and contains the same `SuitTokenFace` SVG
  ...
  Keep the whole token square under flex layout. Do not add a separate CSS
  background circle, directional rim shading, or context-specific image sizing.
...
- Browser preferences persist separately in local storage: opponent selection,
  deck-map visibility/interactivity, log visibility, and animations. The game
  log defaults to hidden for users without a stored preference and is
  remembered once toggled. A restored
