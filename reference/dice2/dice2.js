...
const Config = {
  ...
  faceNumbers: [
    9, 6, 16, 3, 19, 14, 11, 1, 17, 8, 18, 5, 15, 12, 2, 4, 13, 7, 10, 20,
  ],
...
const D20 = {
  ...
  init: function (scene) {
    ...
    const lineGeometry = new THREE.LineSegmentsGeometry().setPositions(
    ...
    for (let i = 0; i < positionAttribute.count; i += 3) {
      ...
      context.fillText(
        D20.faceNumbers[i / 3].toString(),
        Config.labelCanvasSize / 2,
        Config.labelCanvasSize / 2 + 1
      );
...
const RendererModule = {
  ...
  init: function () {
    ...
    const d = 100;
    RendererModule.camera = new THREE.OrthographicCamera(
      -d * aspect,
      d * aspect,
      d,
      -d,
    ...
    RendererModule.d20RenderTarget = new THREE.WebGLRenderTarget(
      window.innerWidth,
      window.innerHeight
    );
    ...
    RendererModule.d20Scene.background = new THREE.Color(
    ...
    const renderPass = new THREE.RenderPass(
      RendererModule.d20Scene,
      RendererModule.camera
    );
    ...
    // Configure CRT pass
    crtPass.uniforms['resolution'].value = new THREE.Vector2(
      window.innerWidth,
      window.innerHeight
    );
    ...
    RendererModule.d20Composer = new THREE.EffectComposer(
  ...
  render: function () {
    ...
    RendererModule.renderer.render(
      RendererModule.labelScene,
      RendererModule.camera
    );
  ...
  updateSize: function () {
    ...
    RendererModule.renderer.setSize(window.innerWidth, window.innerHeight);
    RendererModule.pixelateShader.uniforms.resolution.value.set(
      window.innerWidth,
      window.innerHeight
    );
    ...
    RendererModule.d20RenderTarget.setSize(
      window.innerWidth,
      window.innerHeight
    );

    // Update CRT resolution
    RendererModule.crtPass.uniforms['resolution'].value.set(
      window.innerWidth,
      window.innerHeight
    );
  },
    ...
    if (faceIndex >= 0 && faceIndex < labels.length) {
      ...
      if (sprite) {
        ...
        if (LabelModule.currentFlashedSprite && LabelModule.originalColor) {
          LabelModule.currentFlashedSprite.material.color.copy(
            LabelModule.originalColor
          );
        }
...
const Animation = {
  ...
  rollD20: function (d20) {
    ...
    Animation.angularVelocityX =
      (Math.random() * 2 - 1) * Config.initialAngularVelocity;
    Animation.angularVelocityY =
      (Math.random() * 2 - 1) * Config.initialAngularVelocity;
    Animation.angularVelocityZ =
      (Math.random() * 2 - 1) * Config.initialAngularVelocity;
  ...
  animate: function (d20, labels, camera) {
    ...
    if (Animation.isRolling) {
      ...
      if (t < 1) {
        ...
        d20.scale.set(
          Config.initialScale - Animation.currentScale,
          Config.initialScale - Animation.currentScale,
          Config.initialScale - Animation.currentScale
        );
